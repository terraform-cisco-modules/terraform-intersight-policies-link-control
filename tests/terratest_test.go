...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-link-ctrl-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Link Control Policy.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/fabric/LinkControlPolicies/%s", moid), expectedJSONTemplate, vars)
}
